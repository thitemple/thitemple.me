...
<div class="min-h-screen bg-(--color-bg) font-['Lora'] text-(--color-text)">
	...
	>
		...
		<!-- Mobile menu -->
		<div
			class="grid bg-(--color-bg) transition-all duration-300 ease-in-out md:hidden"
				...
				<div class="flex flex-col gap-6 px-8 pb-6 pt-4 font-['Kantumruy_Pro'] text-lg font-medium">
					...
					<li>
						<button
							...
							onclick={openSearchDialog}
							aria-label="Open search"
							class="text-(--color-text) transition-colors hover:text-(--color-primary)"
						>
							Search
						</button>
					</li>
	...
	{#if searchDialogOpen}
		...
		>
			<button
				...
				class="fixed inset-0 bg-(--color-bg)/75 backdrop-blur-sm"
				...
				>
					...
					<form onsubmit={handleSearchSubmit} class="space-y-4">
						<input
							...
							class="w-full rounded-xl border border-white/20 bg-slate-950/60 px-5 py-3.5 text-white outline-none transition-colors placeholder:text-slate-400 focus:border-(--accent)"
						/>
						<div class="flex items-center justify-end gap-3">
							...
							<button
								type="submit"
								class="font-heading rounded-lg bg-(--accent) px-4 py-2 text-sm font-semibold text-white transition-transform hover:scale-[1.02]"
							>
								Show Results
							</button>
						</div>
	...
	<!-- Footer -->
	<footer class="border-t border-(--color-text)/10 py-10">
		<div
			class="mx-auto flex max-w-250 flex-col items-center justify-between gap-6 px-6 md:flex-row"
		>
			<div class="flex items-center gap-6">
				<a
					...
					rel="noopener noreferrer"
					aria-label="BlueSky"
					class="text-(--color-text) transition-all hover:-translate-y-1 hover:text-(--color-primary)"
				>
				...
				<a
					...
					class="text-(--color-text) transition-all hover:-translate-y-1 hover:text-(--color-primary)"
				>
					<svg viewBox="0 0 24 24" fill="currentColor" class="h-5.5 w-5.5">
						<path
							d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"
						/>
					</svg>
