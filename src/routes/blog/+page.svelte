...
<section class="relative overflow-hidden py-20 lg:py-24">
	...
	<div class="pointer-events-none absolute inset-0">
		<div class="absolute inset-0 bg-[radial-gradient(circle_at_20%_80%,rgba(120,119,198,0.15)_0%,transparent_50%)]"></div>
		<div class="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(255,119,198,0.15)_0%,transparent_50%)]"></div>
	</div>
	
	<div class="relative z-10 mx-auto max-w-7xl px-6 text-center">
		<h1 class="gradient-text mb-6 font-grotesk text-5xl font-extrabold lg:text-6xl">
			Blog
		</h1>
		<p class="mx-auto max-w-2xl text-xl leading-relaxed text-slate-300 lg:text-2xl">
			Practical insights on development, AI-augmented workflows, and building meaningful solutions 
			when you can't burn the midnight oil anymore.
		</p>
	</div>
</section>
		...
		{#if data.posts.length === 0}
			...
			<div class="flex min-h-[400px] items-center justify-center">
				<div class="max-w-2xl text-center">
					...
					<h2 class="mb-4 font-grotesk text-3xl font-bold text-white">
						Starting Fresh
					</h2>
					<p class="text-lg leading-relaxed text-slate-300">
						I'm currently working on new content for this blog. 
						Stay tuned for upcoming posts about development, AI-augmented workflows, 
						and building meaningful solutions in the modern tech landscape.
					</p>
				</div>
			</div>
		{:else}
			...
			<div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
				{#each data.posts as post, idx}
					<article class="group relative overflow-hidden rounded-2xl border border-white/10 bg-black/60 backdrop-blur-sm transition-all hover:-translate-y-2 hover:border-[var(--accent)]/30 hover:shadow-2xl">
						<!-- Gradient overlay on hover -->
						<div class="absolute inset-0 bg-gradient-to-br from-purple-600/8 via-[var(--accent)]/8 to-transparent opacity-0 transition-opacity group-hover:opacity-100"></div>
						...
						{:else}
							<div class="relative flex h-48 items-center justify-center bg-gradient-to-br from-purple-600/30 via-[var(--accent)]/30 to-transparent text-6xl">
								{getPostEmoji(idx)}
							</div>
						{/if}
						...
						<div class="relative z-10 p-6">
							...
							<!-- Title -->
							<h2 class="mb-3 font-grotesk text-2xl font-bold leading-tight text-white">
							...
							{#if post.categories && post.categories.length > 0}
								<div class="mb-5 flex flex-wrap gap-2">
									{#each post.categories.slice(0, 3) as category}
										<span class="rounded-full bg-[var(--accent)]/20 px-3 py-1 text-xs font-medium text-purple-300">
											{category}
										</span>
									{/each}
								</div>
				...
				<div class="hidden items-center gap-2 md:flex">
					{#each Array(data.pageInfo.totalPages) as _, i}
						{#if i + 1 === data.pageInfo.currentPage}
							<span class="rounded-xl bg-gradient-to-r from-[var(--accent)] to-pink-500 px-5 py-3 font-medium text-white">
								{i + 1}
							</span>
